...
import { PiggyBank, Plus, Edit2, Trash2, Check, X, AlertTriangle } from 'lucide-react';
...
export function Budget({ expenses, budgets, categories, onUpdateBudget, onDeleteBudget, onAddCategory }: BudgetProps) {
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className={`rounded-xl p-4 text-white ${
            totalPercentage >= 100 ? 'bg-gradient-to-br from-red-500 to-red-600' :
            totalPercentage >= 80 ? 'bg-gradient-to-br from-orange-500 to-orange-600' :
            'bg-gradient-to-br from-green-500 to-green-600'
          }`}>
            <p className="text-white/90 text-sm mb-1">Remaining</p>
            <p className="text-2xl">${Math.max(0, totalBudget - totalSpent).toFixed(2)}</p>
          </div>
        </div>
      </div>
      ...
      {showAddCategory && (
        <div className="bg-white rounded-2xl shadow-lg p-6 border-2 border-green-200">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm text-gray-600 mb-2">Monthly Budget (Optional)</label>
                  ...
                  <div>
                    ...
                    {spent > budget && (
                      <div className="mt-3 flex items-center gap-2 text-red-600 bg-red-50 px-3 py-2 rounded-lg">
                        ...
                        <span className="text-sm">Over budget by ${(spent - budget).toFixed(2)}</span>
                      </div>
                    )}
                  </div>
                ) : (
                  <div className="text-center py-4 bg-gray-50 rounded-lg">
                    <p className="text-sm text-gray-500 mb-2">No budget set for this category</p>
                    <button
                      onClick={() => handleEditBudget(category)}
